...
class ProductManager {
  constructor() {
    this.path = 'products.json'; // Ajusta el nombre del archivo según tus necesidades
    this.products = [];
  }
  ...
  async init() {
    try {
      ...
      console.log(this.getProductById(newProduct.id));

      try {
        console.log(this.getProductById(123));
      } catch (error) {
        console.error(error.message);
      }

      await this.updateProduct(newProduct.id, { price: 250 });
      console.log(this.getProductById(newProduct.id));
...
// Crear una instancia de ProductManager y ejecutar la función init
